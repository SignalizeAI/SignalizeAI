...
const getVisibleItems = () =>
  qsa('#saved-list .saved-item').filter((item) => !item.classList.contains('pending-delete'));
...
export function updateDeleteState() {
  const { multiSelectToggle } = getElements();
  if (!multiSelectToggle) return;

  const countIndicator = byId('selection-count-indicator');
  const count = state.selectedSavedIds.size;
  const totalVisible = getVisibleItems().length;
...
export function updateSavedActionsVisibility(count) {
  const searchToggleBtn = byId('search-toggle');
  const filterToggle = byId('filter-toggle');
  const exportToggle = byId('export-menu-toggle');
  const { multiSelectToggle } = getElements();
...
export function updateSelectionUI() {
  const { selectionBackBtn, selectAllBtn } = getElements();

  qsa('.saved-item').forEach((item) => toggleItemSelectionUI(item, state.selectionMode));

  byId('export-menu-toggle')?.classList.toggle('hidden', state.selectionMode);
  byId('filter-toggle')?.classList.toggle('hidden', state.selectionMode);
...
export function exitSelectionMode() {
  const { selectAllBtn } = getElements();
  ...
  qsa('.saved-item.selected').forEach((el) => el.classList.remove('selected'));
