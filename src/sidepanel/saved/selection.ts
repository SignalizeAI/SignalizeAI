...
export function updateSelectionUI(): void {
  ...
  document
    .querySelectorAll<HTMLElement>('.saved-item')
...
export function toggleSelectAllVisible(): void {
  ...
  const selectedCount = items.filter(
    (item) => item.querySelector<HTMLInputElement>('.saved-select-checkbox')?.checked
  ).length;

  const shouldSelectAll = selectedCount < items.length;

  items.forEach((item) => {
    const cb = item.querySelector<HTMLInputElement>('.saved-select-checkbox');
    if (!cb) return;

    if (cb.checked !== shouldSelectAll) {
      cb.checked = shouldSelectAll;
      const wrapper = item.closest<HTMLElement>('.saved-item');
      if (wrapper) {
