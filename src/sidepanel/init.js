...
export function initSidepanel() {
    ...
    switch (message.type) {
      case "TAB_CHANGED": {
        ...
        if (!enabled && state.currentView === "analysis") {
          ...
          const isIrrelevant = IRRELEVANT_DOMAINS.some((domain) =>
      ...
      const { data: insertData, error } = await supabase
        ...
        .insert({
          ...
          recommended_outreach_persona:
            state.lastAnalysis.recommendedOutreach?.persona,
  ...
  filterApplyBtn?.addEventListener("click", async () => {
    ...
    state.activeFilters.persona =
      document.getElementById("filter-persona")?.value.toLowerCase().trim();
  ...
  document
    .getElementById("modal-upgrade-team-btn")
