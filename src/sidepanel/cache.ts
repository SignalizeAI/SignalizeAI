...
export async function hashContent(content: WebsiteContent): Promise<string> {
  const text = [
    content.title,
...
function makeDomainCacheKey(domain: string): string {
  const rootDomain = extractRootDomain(domain);
  return `analysis_cache:domain:${rootDomain}`;
}

function makeDomainAnalyzedTodayKey(domain: string): string {
  const rootDomain = extractRootDomain(domain);
  return `domain_analyzed_today:${rootDomain}`;
}

export async function wasDomainAnalyzedToday(domain: string): Promise<boolean> {
...
export function markDomainAnalyzedToday(domain: string): void {
  const key = makeDomainAnalyzedTodayKey(domain);
  chrome.storage.local.set({ [key]: { timestamp: Date.now() } });
}

export async function getCachedAnalysis(url: string): Promise<CachedEntry | null> {
  return new Promise((resolve) => {
...
export async function getCachedAnalysisByDomain(domain: string): Promise<CachedEntry | null> {
  return new Promise((resolve) => {
